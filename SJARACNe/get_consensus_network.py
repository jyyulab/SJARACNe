    ...
    with open(sys.argv[1] + adj_file, "r") as f:  # Opening each bootstrap file
        for line in f:
            ...
            if line[0] != ">":  # Processing non header lines representing the network
                tokens = line.split(
                    "\t"
                )  # Tokenizing each non header line with tab delimmiter
                hub_id = tokens[0]  # First token is the hub id
                for i in range(
                    1, len(tokens), 2
                ):  # Iterating on all adjacent genes: Odd indexes are the connected genes and even indexes are the corresponding value to the edge between the hub gene and the gene with an odd index appearing before the value in the tokens list
                    ...
                    if key in total_support:
                        total_support[
                            ...
                            1
                        )  # Updating the total number of edges observed for the particualr key (edge)
                        total_mi[key] += float(
                            tokens[i + 1]
                        ...
                        total_support[
                            ...
                            1
                        )  # Initializing the total number of edges observed for the particular key (edge) if the key (edge) is newly generated
                        total_mi[key] = float(
                            tokens[i + 1]
                        )  # Initializing total MI between the genes involving in the particulat key (edge) if the key (edge) is newly generated
                    total_edge[
                        run_num
                    ] += 1  # Increment the total number of edges processed so far
