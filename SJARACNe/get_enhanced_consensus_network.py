    ...
    for tar_gene in marker_set.keys():
        if src_gene != tar_gene:
            if src_gene + "--" + tar_gene in gene_network:
                ...
                rho, intercept, r, p, stderr = stats.linregress(
                    src_samples, tar_samples
                )
                ...
                gene_network[src_gene + "--" + tar_gene] = dict(
                    srcSymbol=src_symbol,
                    tarSymbol=tar_symbol,
                    mi=mi,
                    rho=rho,
                    intercept=intercept,
                    r=r,
...
out_name = (
    "_".join(input_net_name_tokens[0:2])
    + "_ncol_"
...
for edge in gene_network.keys():
    ...
    data = (
        ...
        info["tarSymbol"],
        "{0:.4f}".format(info["mi"]),
        "{0:.4f}".format(info["pcc"]),
        "{0:.4f}".format(info["scc"]),
        "{0:.4f}".format(info["rho"]),
        "{0:.4f}".format(info["p"]),
    )
    out.write("\t".join(data) + "\n")
    ...
    if out_subnet != None:
        if (
            info["srcSymbol"].strip() in subnet_list
            or info["tarSymbol"].strip() in subnet_list
        ):
